...
public class AuthDAO {
    ...
    // ══ Resultado extendido de loginCompleto ══════════════════════════════════
    public enum LoginResult {
        SUCCESS,          // Login correcto
        INVALID_CREDENTIALS, // Usuario/contraseña incorrectos
        ACCOUNT_INACTIVE, // Cuenta no activada
        BLOCKED,          // Bloqueado por demasiados intentos
        ERROR             // Error técnico
    }
    ...
    // ════════════════════════════════════════════════════════════════════════
    // LOGIN PRINCIPAL — devuelve LoginResult para que el controller
    // pueda mostrar mensajes específicos (bloqueado, inactivo, etc.)
    // ════════════════════════════════════════════════════════════════════════

    public static LoginResult loginCompleto(String identifier, String plainPassword) {
        LoginAttemptService attemptSvc = LoginAttemptService.getInstance();

        // 1. ¿Está bloqueado?
        if (attemptSvc.isBlocked(identifier)) {
            System.out.println("🔒 [AuthDAO] Login bloqueado para: " + identifier);
            return LoginResult.BLOCKED;
        }

        String sql = "SELECT id, username, email, password_hash, is_active " +
                     "FROM users WHERE username = ? OR email = ?";

        try (Connection conn = DatabaseConnection.getConnection()) {
            if (conn == null) return LoginResult.ERROR;
                ...
                try (ResultSet rs = ps.executeQuery()) {
                    if (rs.next()) {
                        ...
                        if (BCrypt.checkpw(plainPassword, hash)) {
                            ...
                            attemptSvc.resetAttempts(identifier);

                            GeoData geo    = fetchGeoData();
                            String pcName  = limit(InetAddress.getLocalHost().getHostName(), 50);
                            String osInfo  = limit(System.getProperty("os.name") + " " +
                                                   System.getProperty("os.version"), 50);
                            ...
                            if (sessionId > 0) {
                                ...
                                registrarAuditLog(conn, userId, "LOGIN_SUCCESS",
                                    "Acceso desde " + geo.city + ", " + geo.country,
                                    geo.publicIp, pcName, osInfo);
                            ...
                            registrarLogLogin(conn, userId, 0, false, "Contraseña incorrecta", "0.0.0.0");

                            boolean recienBloqueado = attemptSvc.registerFailedAttempt(identifier);

                            if (recienBloqueado) {
                                // Enviar email de alerta en hilo separado para no bloquear el login task
                                final String emailDestino = userEmail;
                                final String ipInfo = fetchGeoData().publicIp;
                                new Thread(() -> {
                                    try {
                                        new EmailService().sendSecurityAlert(emailDestino, ipInfo);
                                        System.out.println("📧 [AuthDAO] Email de alerta enviado a: " + emailDestino);
                                    } catch (Exception ex) {
                                        System.err.println("⚠️ No se pudo enviar email de alerta: " + ex.getMessage());
                                    }
                                }, "email-alert-thread").start();

                                registrarAuditLog(conn, userId, "ACCOUNT_LOCKED",
                                    "Cuenta bloqueada por 3 intentos fallidos", "0.0.0.0", "—", "—");
                                return LoginResult.BLOCKED;
                            }
        ...
        attemptSvc.registerFailedAttempt(identifier);
        return LoginResult.INVALID_CREDENTIALS;
    }

    // ════════════════════════════════════════════════════════════════════════
    // MÉTODO LEGACY — mantiene compatibilidad con código existente que
    // llame a loginCompleto esperando boolean.
    // ════════════════════════════════════════════════════════════════════════

    /** @deprecated Usar {@link #loginCompleto(String, String)} que devuelve LoginResult */
    public static boolean loginCompletoBool(String identifier, String plainPassword) {
        return loginCompleto(identifier, plainPassword) == LoginResult.SUCCESS;
    }

    // ════════════════════════════════════════════════════════════════════════
    // MÉTODOS PRIVADOS (sin cambios respecto al original)
    // ════════════════════════════════════════════════════════════════════════
    ...
    private static void registrarLogLogin(Connection conn, int userId, int deviceId,
                                          boolean success, String reason, String ip) {
    ...
    private static void registrarAuditLog(Connection conn, int userId, String action,
                                          String desc, String ip, String pc, String os) {
    ...
    // ════════════════════════════════════════════════════════════════════════
    // REGISTRO / CLASES / RECUPERACIÓN (sin cambios)
    // ════════════════════════════════════════════════════════════════════════
    ...
    public void updateUserClass(int userId, int classId) throws SQLException {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            pstmt.setInt(1, classId); pstmt.setInt(2, userId); pstmt.executeUpdate();
        }
    }
    ...
    public String generateResetCode(String email) throws SQLException {
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            try {
                ...
                String sql = "INSERT INTO email_verifications (user_id, email, token, expires_at) VALUES (?, ?, ?, NOW() + INTERVAL '15 minutes')";
                try (PreparedStatement ps = conn.prepareStatement(sql)) {
                    ps.setInt(1, userId); ps.setString(2, limit(email, 100)); ps.setString(3, token); ps.executeUpdate();
                }
    ...
    public boolean verifyResetCode(String email, String code) throws SQLException {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, limit(email, 100)); ps.setString(2, code.trim());
    ...
    public boolean resetPassword(String email, String newPassword) throws SQLException {
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            conn.setAutoCommit(false);
            try {
                try (PreparedStatement ps = conn.prepareStatement("UPDATE users SET password_hash = ? WHERE email = ?")) {
                    ps.setString(1, passwordHash); ps.setString(2, limit(email, 100));
